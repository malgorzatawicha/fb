...
<form action="{{ route('admin.cms.pages.update', [$page->slug]) }}" method="POST" class="form-horizontal page-form"  enctype="multipart/form-data">
        ...
        <div class="col-sm-9">
            <select name="type" id="page-type" class="form-control">
                @foreach($types as $type)
                    <option value="{{$type}}" @if($type == $page->type) selected @endif>{{trans('admin.pages.types.' . $type)}}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <div class="form-group">
        <div class="checkbox col-sm-offset-2">
            ...
            <label><input type="checkbox" @if (!empty($page->active)) checked="checked" @endif name="active" id="page-active" value="1">{{trans('admin.pages.active')}}</label>
        </div>
    </div>
    <div class="form-group">
