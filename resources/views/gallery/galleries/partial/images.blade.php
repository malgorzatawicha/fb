<div class="panel-heading">
    <div class="pull-right">
        <a class="btn btn-primary" title="{{ trans('admin.create') }}" href="#"
           ...
           data-submit-action="{{ route('admin.galleries.images.store', [$gallery->slug]) }}"
        ...
        <div class="row gallery-images">
            @foreach($gallery->images as $image )
                <div class="col-sm-3 thumb">
                        <a class="thumbnail" href="{{route('admin.galleries.images.show', ['gallery' => $gallery->slug, 'image' =>$image->id])}}">
                            <img class="img-responsive" src="{{$image->image_thumbnail_path }}{{ $image->image_thumbnail_filename . '?'. 'time='. time() }}">
                        </a>
                    <form action="{{route('admin.galleries.images.destroy', ['gallery' => $gallery->slug, 'image' => $image->id]) }}" method="POST">
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
...
@include('gallery.galleries.partial.images.create_modal')
@include('gallery.galleries.partial.images.edit_modal')